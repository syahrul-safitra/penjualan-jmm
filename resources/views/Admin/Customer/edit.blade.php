                ...
                <form action="{{ url('customer/' . $customer->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                            id="name" value="{{ @old('name', $customer->name) }}" autocomplete="off" autofocus>
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" min="1" class="form-control @error('email') is-invalid @enderror"
                            name="email" id="email" value="{{ @old('email', $customer->email) }}" autocomplete="off">
                        @error('email')
                            <div class="invalid-feedback text-red">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" min="0" class="form-control @error('no_wa') is-invalid @enderror"
                            name="no_wa" id="no_wa" value="{{ @old('no_wa', $customer->no_wa) }}" autocomplete="off">
                        @error('no_wa')
                            <div class="invalid-feedback text-red">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('alamat') is-invalid @enderror" name="alamat"
                            id="alamat" value="{{ @old('alamat', $customer->alamat) }}" autocomplete="off">
                        @error('alamat')
                            <div class="invalid-feedback text-red">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <img class="mt-4" id="img-preview" style="width: 200px; height: 200px"
                            src="{{ asset('file/' . $customer->gambar) }}">
                        @error('gambar')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>

                    <a href="{{ url('/customers') }}" class="btn btn-warning me-2">Batal</a>
