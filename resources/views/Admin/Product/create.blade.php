                ...
                <form action="{{ url('product') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" min="1" class="form-control @error('harga') is-invalid @enderror"
                            name="harga" id="harga" value="{{ @old('harga') }}" autocomplete="off">
                        @error('harga')
                            <div class="invalid-feedback text-red">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" min="0" class="form-control @error('jumlah') is-invalid @enderror"
                            name="jumlah" id="jumlah" value="{{ @old('jumlah') }}" autocomplete="off">
                        @error('jumlah')
                            <div class="invalid-feedback text-red">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('deskripsi') is-invalid @enderror" name="deskripsi" id="deskripsi"
                            value="{{ @old('deskripsi') }}" maxlength="200" placeholder="max 200 karakter"></textarea>
                        @error('deskripsi')
                            <div class="invalid-feedback text-red">{{ $message }}</div>
                        @enderror
                    </div>
